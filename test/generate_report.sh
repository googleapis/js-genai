...
# GAOS is generated by Speakeasy; do not let generated surface expansion move
# the handwritten SDK coverage gate.
C8_EXCLUDE_ARGS=(
  "--exclude=src/private/**"
  "--exclude=dist/src/private/**"
  "--exclude=src/gaos/**"
  "--exclude=dist/src/gaos/**"
)
...
c8 "${C8_EXCLUDE_ARGS[@]}" \
  --reporter=json \
  --report-dir="${WORK_DIR}/${UNIT}" \
  jasmine dist/test/unit/**/*_test.js dist/test/unit/*_test.js
c8 "${C8_EXCLUDE_ARGS[@]}" \
  --reporter=json \
  --report-dir="${WORK_DIR}/${SYSTEM}" \
  jasmine dist/test/system/node/*_test.js -- --test-server
