...
describe('generateContent', () => {
  describe('can pick out the types.Schema and pass to responseSchema field and give the rest to responseJsonSchema field', () => {
    it('should give the types.Schema to responseSchema field', async () => {
    ...
    it('should give the schema that is not types.Schema to responseJsonSchema field', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
    ...
    it('will throw an error if both responseSchema and responseJsonSchema fields are set', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      await expectAsync(
  ...
  describe('can pick out the types.Schema and pass to parameters/response field and give the rest to parametersJsonSchema/responseJsonSchema field', () => {
    it('should give the types.Schema to parameters and response field', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      const shouldBeGivenToParameters: types.Schema = {
        type: types.Type.OBJECT,
        properties: {
      ...
      const shouldBeGivenToResponse: types.Schema = {
        type: types.Type.OBJECT,
        properties: {
          firstString: {type: types.Type.STRING},
          secondString: {type: types.Type.STRING},
      ...
      const fd: types.FunctionDeclaration = {
        ...
        parameters: shouldBeGivenToParameters,
        response: shouldBeGivenToResponse,
      };
      const fetchSpy = spyOn(global, 'fetch').and.returnValue(
        Promise.resolve(
          new Response(
      ...
      expect(parsedTool['parameters']).toEqual(shouldBeGivenToParameters);
      expect(parsedTool['parametersJsonSchema']).toBeUndefined();
      expect(parsedTool['response']).toEqual(shouldBeGivenToResponse);
      expect(parsedTool['responseJsonSchema']).toBeUndefined();
    });
    it('should give the data that is not types.Schema to parametersJsonSchema field', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      ...
      const fd: types.FunctionDeclaration = {
        ...
        parameters: shouldBeGivenToParametersJsonSchema as Record<
          ...
          unknown
        >,
        response: shouldBeGivenToResponseJsonSchema as Record<string, unknown>,
      };
      const fetchSpy = spyOn(global, 'fetch').and.returnValue(
        Promise.resolve(
          new Response(
    ...
    it('should be no-op when provided the parametersJsonSchema field', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      const shouldBeGivenToParametersJsonSchema: unknown = {
        type: types.Type.OBJECT,
        properties: {
      ...
      const fd: types.FunctionDeclaration = {
        name: 'concatStringFunction',
        description: 'this is a concat string function',
        parametersJsonSchema: shouldBeGivenToParametersJsonSchema,
        responseJsonSchema: shouldBeGivenToResponseJsonSchema,
      };
      const fetchSpy = spyOn(global, 'fetch').and.returnValue(
        Promise.resolve(
    ...
    it('will throw an error if both parameters and parametersJsonSchema fields are set', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      await expectAsync(
        client.models.generateContent({
          model: 'gemini-1.5-flash-exp',
        ...
        new Error(
          'In function declaration concatStringFunction, Only one of parameters and parametersJsonSchema can be set. Please set parametersJsonSchema.',
        ),
      );
    });
    it('will throw an error if both response and responseJsonSchema fields are set', async () => {
      const client = new GoogleGenAI({vertexai: false, apiKey: 'fake-api-key'});
      await expectAsync(
        client.models.generateContent({
          model: 'gemini-1.5-flash-exp',
        ...
        new Error(
          'In function declaration concatStringFunction, Only one of response and responseJsonSchema can be set. Please set responseJsonSchema.',
        ),
      );
    });
  });
