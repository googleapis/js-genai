...
import * as ProcessorModule from '../../../../src/cross/sentencepiece/_processor.js';
import {SentencePieceProcessor, Token,} from '../../../../src/cross/sentencepiece/_processor.js';
import {TokenizerCache, TokenizerFileSystem, TokenizerPlatform,} from '../../../../src/cross/tokenizer/_interfaces.js';
...
describe('LocalTokenizer', () => {
  ...
  describe('countTokens', () => {
    ...
    beforeEach(() => {
      ...
      const MockProcessorCtor =
          jasmine.createSpy('ProcessorCtor').and.returnValue(mockProcessor);

      tokenizer = new LocalTokenizer(
          'gemini-2.0-flash-001',
          mockPlatform,
          MockProcessorCtor as any,
      );
    });
  ...
  describe('model loading', () => {
    ...
    it('should throw error if model download fails validation', async () => {
      ...
      await expectAsync(tokenizer.countTokens('Test'))
  ...
  describe('error handling', () => {
    ...
    it('should throw error for non-text content in countTokens', async () => {
      ...
      await expectAsync(tokenizer.countTokens(content))
          .toBeRejectedWithError(
    ...
    it('should throw error for non-text content in computeTokens', async () => {
      ...
      await expectAsync(tokenizer.computeTokens(content))
          .toBeRejectedWithError(
              /LocalTokenizers do not support non-text content types/,
          );
    });
