...
/**
 * A subset of JSON Schema according to 2020-12 JSON Schema draft. With one
 * additional google only field: propertyOrdering
 *
...
export interface JSONSchema {
  ...
  /**
   * The order of the properties. Not a standard field in open api spec.
    ...
    if (fieldName == 'type') {
      ...
      for (const item of fieldValue) {
        ...
        listSchemaFieldValue.push(
          processJsonSchema(item as JSONSchema) as types.Schema,
        );
      }
...
export function tSchema(
  apiClient: ApiClient,
  schema: types.Schema | Record<string, unknown>,
): types.Schema {
    ...
    for (const functionDeclaration of tool.functionDeclarations) {
      ...
      if (functionDeclaration.response) {
        functionDeclaration.response = tSchema(
          apiClient,
          functionDeclaration.response as Record<string, unknown>,
        );
      }
    }
  }
