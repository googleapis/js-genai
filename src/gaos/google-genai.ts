...
export class GeminiNextGenEnvironmentFiles {
  ...
  async upload(
    ...
    let reqApiVersion =
      params.api_version ?? (options as any)?.httpOptions?.apiVersion;
    if (!reqApiVersion) {
      try {
        reqApiVersion =
          typeof apiClient.getApiVersion === 'function'
            ? apiClient.getApiVersion()
            : apiClient.apiVersion;
      } catch {
        reqApiVersion = 'v1alpha';
      }
    }
    reqApiVersion = (reqApiVersion || 'v1alpha').replace(/^\/+/, '');
    ...
    const handshakeResponse = await apiClient.request({
      ...
      httpOptions: {
        ...(options as any)?.httpOptions,
        apiVersion: `upload/${reqApiVersion}`,
