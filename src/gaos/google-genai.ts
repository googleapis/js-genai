...
export class GeminiNextGenInteractions {
  ...
  async create(
    ...
    GoogleGenAIInteraction | Stream<GoogleGenAIInteractionSSEEvent>
  > {
    const { api_version, ...request } = params;
    if (request.stream === true) {
      const response = await this.getClient(api_version).interactions.create(
        {
      ...
      return response as Stream<GoogleGenAIInteractionSSEEvent>;
    }
  ...
  > {
    ...
    if (stream === true) {
      const response = await this.getClient(api_version).interactions.get(
        id,
  ...
  async delete(
    ...
    return this.getClient(params?.api_version).interactions.delete(
      id,
