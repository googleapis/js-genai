...
function convertBidiSetupToTokenSetup(requestDict, config) {
    ...
    if (setupForMaskGeneration) {
        ...
        if (Array.isArray(config?.lockAdditionalFields) &&
        ...
        else if (config?.lockAdditionalFields &&
            config.lockAdditionalFields.length > 0 &&
...
export class Tokens extends BaseModule {
    ...
    async create(params) {
        ...
        if (this.apiClient.isVertexAI()) {
            ...
            const body = converters.createAuthTokenParametersToMldev(this.apiClient, params);
            path = common.formatMap('auth_tokens', body['_url']);
