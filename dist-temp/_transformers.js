...
export function tModel(apiClient, model) {
...
export function tCachesModel(apiClient, model) {
...
export function tBlob(blob) {
    if (typeof blob === 'object' && blob !== null) {
        return blob;
    }
    throw new Error(`Could not parse input as Blob. Unsupported blob type: ${typeof blob}`);
}
export function tImageBlob(blob) {
...
export function tAudioBlob(blob) {
    const transformedBlob = tBlob(blob);
    if (transformedBlob.mimeType &&
...
export function tContent(origin) {
    if (origin === null || origin === undefined) {
...
export function tContentsForEmbed(apiClient, origin) {
    if (!origin) {
        return [];
    }
...
export function tContents(origin) {
    if (origin === null ||
        origin === undefined ||
        (Array.isArray(origin) && origin.length === 0)) {
...
function flattenTypeArrayToAnyOf(typeList, resultingSchema) {
    ...
    if (listWithoutNull.length === 1) {
        resultingSchema['type'] = Object.values(types.Type).includes(listWithoutNull[0].toUpperCase())
        ...
        for (const i of listWithoutNull) {
            resultingSchema['anyOf'].push({
                'type': Object.values(types.Type).includes(i.toUpperCase())
                    ? i.toUpperCase()
                    : types.Type.TYPE_UNSPECIFIED,
            });
        }
...
export function processJsonSchema(_jsonSchema) {
    const genAISchema = {};
    ...
    */
    ...
    for (const [fieldName, fieldValue] of Object.entries(_jsonSchema)) {
        ...
        if (fieldName == 'type') {
            ...
            genAISchema['type'] = Object.values(types.Type).includes(fieldValue.toUpperCase())
...
export function tSpeechConfig(speechConfig) {
...
export function tLiveSpeechConfig(speechConfig) {
    if ('multiSpeakerVoiceConfig' in speechConfig) {
        throw new Error('multiSpeakerVoiceConfig is not supported in the live API.');
    }
    return speechConfig;
}
export function tTool(tool) {
...
export function tCachedContentName(apiClient, name) {
    if (typeof name !== 'string') {
        throw new Error('name must be a string');
    }
    return resourceName(apiClient, name, 'cachedContents');
}
export function tTuningJobStatus(status) {
    switch (status) {
...
export function tBytes(fromImageBytes) {
...
export function isGeneratedVideo(origin) {
    return (origin !== null &&
        origin !== undefined &&
        typeof origin === 'object' &&
        'video' in origin);
}
export function isVideo(origin) {
    return (origin !== null &&
        origin !== undefined &&
        typeof origin === 'object' &&
        'uri' in origin);
}
export function tFileName(fromName) {
    let name;
    if (_isFile(fromName)) {
        name = fromName.name;
    }
...
export function tModelsUrl(apiClient, baseModels) {
    let res;
    if (apiClient.isVertexAI()) {
...
export function tExtractModels(response) {
...
export function mcpToGeminiTool(mcpTool, config = {}) {
 ...
 * declarations.
 */
export function mcpToolsToGeminiTool(mcpTools, config = {}) {
...
export function tBatchJobSource(client, src) {
    let sourceObj;
...
export function tEmbeddingBatchJobSource(client, src) {
    if (client.isVertexAI()) {
        throw new Error('Embedding batch jobs are not supported in Vertex AI.');
    }
    const sourceObj = { ...src };
    const mldevSources = Number(!!sourceObj.inlinedRequests) + Number(!!sourceObj.fileName);
    if (mldevSources !== 1) {
        throw new Error('Exactly one of `inlinedRequests` or `fileName` must be set for Embedding Batch Jobs in the Gemini API.');
    }
    return sourceObj;
}
export function tBatchJobDestination(dest) {
...
export function tRecvBatchJobDestination(dest) {
...
export function tBatchJobName(apiClient, name) {
...
export function tJobState(state) {
